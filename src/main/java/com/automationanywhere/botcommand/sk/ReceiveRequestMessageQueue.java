...
@BotCommand
@CommandPkg(label = "Receive Request", name = "ReceivcRequestQueue", description = "Receives request from a queue", 
icon = "pkg.svg", group_label="Request/Reply",  node_label = "Receive Request Queue {{sessionName}}|", comment = true ,  text_color = "#dc7ca3" , background_color =  "#dc7ca3" ,
return_type=DataType.STRING  , return_label="Request", return_required=true)

public class ReceiveRequestMessageQueue {
    ...
    public StringValue action(@Idx(index = "1", type = TEXT) @Pkg(label = "Session Name",  default_value_type = STRING, default_value = "Default") @NotEmpty String sessionName,
    	...
    	String message = connection.getRequestQueue(requestqueue, timeout.intValue(),filter);
